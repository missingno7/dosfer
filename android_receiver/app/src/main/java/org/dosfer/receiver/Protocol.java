...
public final class Protocol {
    public static final int DATA=1, END_WINDOW=2, CALIBRATION=3, CHAIN_XOR=4;
    ...
    public static final int FLAG_REPEATED=0x0001, FLAG_WHITENED=0x0008;
    ...
    public static Frame parseFrame(byte[] raw) {
        ...
        if(ver!=1||hlen!=48||reserved!=0||session==0||(flags&~0x000f)!=0||kind<1||kind>4||raw.length!=48+plen)
        ...
        if((flags&FLAG_WHITENED)!=0)whitenPayload(payload,0,plen,session,global);
    ...
    public static byte[] encodeFrame(int kind,int flags,long session,int window,long global,int wi,int wc,long sid,long off,byte[] plain) {
        if(session==0||kind<1||kind>4||plain==null||plain.length>0xffff)throw new IllegalArgumentException("frame fields");
        ...
        if((flags&FLAG_WHITENED)!=0)whitenPayload(raw,FRAME_HEADER,plain.length,session,global);
