...
int main(void) {
    ...
    u32 triple_baseline=0,triple_scatter=0,triple_full=0;
    u32 fallback_asm=0,range_asm_full=0;
    ...
    u32 parity_derive=0;
    u32 delta_hash,direct_hash,asm_hash,stripe_hash,grouped_hash,phased_hash,
        triple_hash,range_hash,a,b;
    ...
    u16 triple_groups,triple_source_codewords,triple_destination_bytes;
    ...
    vga_rgb3_stripe_triple_stats(&triple_groups,&triple_source_codewords,
        &triple_destination_bytes);
#ifdef DOSFER_STRIPE_DUMP
    for(step=0;step<stripe_runs;++step) {
        u16 first,count,top,bottom;
        u8 mask,flags;
        signed char direction;
        if(vga_rgb3_stripe_run_info(step,&first,&count,&top,&bottom,&mask,
                &direction,&flags))
            printf("STRIPE %u first %u count %u top %u bottom %u mask %02X dir %d flags %u\n",
                step,first,count,top,bottom,mask,(int)direction,flags);
    }
#endif
    ...
    for(step=0;step<LOOPS;step++) {
        ...
        if(!vga_rgb3_direct_reset_template()) {rc=52;goto leave_vga;}
        a=timer_ticks();
        if(!vga_rgb3_direct_scatter_with_triples(
                (const u8 *const *)current_arg)) {rc=53;goto leave_vga;}
        b=timer_ticks();triple_full+=b-a;
        triple_hash=vga_screen_hash();
        if(triple_hash!=stripe_hash) {
            printf("FAIL: RGB triple stripe triplet %u stripe %08lX triple %08lX\n",
                step+1,stripe_hash,triple_hash);rc=54;goto leave_vga;
        }
        if(!vga_rgb3_direct_reset_template()) {rc=59;goto leave_vga;}
        a=timer_ticks();
        if(!vga_rgb3_direct_scatter_range_asm_full(
                (const u8 *const *)current_arg)) {rc=60;goto leave_vga;}
        b=timer_ticks();range_asm_full+=b-a;
        range_hash=vga_screen_hash();
        if(range_hash!=stripe_hash) {
            printf("FAIL: RGB range ASM triplet %u stripe %08lX range %08lX\n",
                step+1,stripe_hash,range_hash);rc=61;goto leave_vga;
        }
    }
    for(step=0;step<LOOPS;++step) {
        ...
        if(!vga_rgb3_direct_reset_template()) {rc=62;goto leave_vga;}
        a=timer_ticks();
        if(!vga_rgb3_direct_scatter_fallback_asm(
                (const u8 *const *)current_arg)) {rc=63;goto leave_vga;}
        b=timer_ticks();fallback_asm+=b-a;
        a=timer_ticks();
        ...
        if(!vga_rgb3_direct_reset_template()) {rc=55;goto leave_vga;}
        a=timer_ticks();
        if(!vga_rgb3_direct_scatter_triple_baseline(
                (const u8 *const *)current_arg)) {rc=56;goto leave_vga;}
        b=timer_ticks();triple_baseline+=b-a;
        if(!vga_rgb3_direct_reset_template()) {rc=57;goto leave_vga;}
        a=timer_ticks();
        if(!vga_rgb3_direct_scatter_triples(
                (const u8 *const *)current_arg)) {rc=58;goto leave_vga;}
        b=timer_ticks();triple_scatter+=b-a;
    }
    ...
    printf("RGB3 integrated optimized production x%u: template+scatter %lu ms (%lu/triplet)\n",
        LOOPS,timer_elapsed_ms(0,production_raster),
        timer_elapsed_ms(0,production_raster)/(u32)LOOPS);
    printf("RGB3 complete optimized production x%u: total %lu ms (%lu/triplet)\n",
    ...
    printf("RGB3 fallback range ASM x%u: C %lu ms, one-codeword ASM %lu ms; complete range-ASM scatter %lu ms (%lu/triplet)\n",
        LOOPS,timer_elapsed_ms(0,isolated_fallback),
        timer_elapsed_ms(0,fallback_asm),timer_elapsed_ms(0,range_asm_full),
        timer_elapsed_ms(0,range_asm_full)/(u32)LOOPS);
    ...
    printf("RGB3 phase groups: %u groups, %u source codewords -> %u destination bytes plus %u edge contributions\n",
        ...
        phase_edge_contributions);
    printf("RGB3 triple groups: %u groups, %u source codewords -> %u destination bytes\n",
        triple_groups,triple_source_codewords,triple_destination_bytes);
    ...
    printf("RGB3 triple isolation x%u: original runs %lu ms, triple byte scatter %lu ms; complete triple scatter %lu ms (%lu/triplet)\n",
        LOOPS,timer_elapsed_ms(0,triple_baseline),
        timer_elapsed_ms(0,triple_scatter),timer_elapsed_ms(0,triple_full),
        timer_elapsed_ms(0,triple_full)/(u32)LOOPS);
