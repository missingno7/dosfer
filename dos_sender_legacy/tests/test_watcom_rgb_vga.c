...
int main(void) {
    ...
    for(triplet=0;triplet<13;triplet++) {
        ...
        if(!vga_apply_codewords3_direct((const u8 *const *)reference_codewords)) {
    ...
    puts("PASS: production RGB3 direct R/G/B rasters match delta and full QR renders through frame 39");
