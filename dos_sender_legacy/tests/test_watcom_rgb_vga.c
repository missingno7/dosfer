/* DOS/Open Watcom oracle for the RGB3 delta renderer.
 *
 * Each delta-rendered R/G/B triplet is compared with an independently
 * full-rendered triplet, both in the RAM raster and in the actual VGA planes.
 * The 13 iterations include the reported frame-39 position. */
#include <stdio.h>
#include <string.h>
#include <malloc.h>
#include "dosfer.h"
...
int main(void) {
    ...
    for(channel=0;channel<3;channel++) {
        ...
        if(!current[channel]||!workspace[channel]||
    ...
    for(triplet=0;triplet<13;triplet++) {
        ...
        if(!vga_apply_v40l_delta3(data,ecc,current_arg)) {
            puts("FAIL: RGB delta application failed");rc=8;goto leave_vga;
        }
    ...
    puts("PASS: RGB3 delta R/G/B codewords, rasters and VGA planes match full QR renders through frame 39");
