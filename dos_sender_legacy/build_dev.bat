...
rem Developer build adds /CAL, /BENCH, VGA readback/hash checks and diagnostics.
wcl -q -bt=dos -ml -3 -ot -ol -oi -or -oh -s -za99 -dDOSFER_DEVTOOLS %EXTRA_CFLAGS% -Iinclude -Ithird_party ^
  -fe=build\DOSFERD.EXE -fm=build\DOSFERD.MAP ^
  src\sender.c src\sender_config.c src\producer.c src\protocol.c src\vga.c src\timing.c third_party\qrcodegen.c
if errorlevel 1 exit /b 1
echo Built developer build\DOSFERD.EXE
