...
DATA, END_WINDOW, CALIBRATION, CHAIN_XOR = 1, 2, 3, 4
SESSION, DIRECTORY, FILE_BEGIN, FILE_DATA, FILE_END, TRANSFER_END = range(1, 7)
FLAG_REPEATED, FLAG_WHITENED = 0x0001, 0x0008
...
class Frame:
    ...
    def encode(self) -> bytes:
        ...
        wire_payload = (whiten_payload(self.payload, self.session, self.global_index)
    ...
    def decode(raw: bytes) -> "Frame":
        ...
        if kind not in (DATA, END_WINDOW, CALIBRATION, CHAIN_XOR): raise ValueError("unknown kind")
        ...
        if flags & FLAG_WHITENED: payload = whiten_payload(payload, session, glob)
...
def chain_frame(left: Frame, right: Frame) -> Frame:
    ...
    return Frame(CHAIN_XOR,FLAG_WHITENED,left.session,left.window,left.global_index,left.window_index,left.window_count,lengths,0,payload)
